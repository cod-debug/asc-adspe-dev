    ...
    <q-scroll-area class="fit text-red-14 text-bold">
      <q-list padding>
        <div class="text-center" v-if="!miniState">
          <h6 class="q-ma-none text-dark">{{ userRole.toUpperCase() }}</h6>
          <hr class="nav-hr" />
        </div>
        <AdSpeDrawer v-if="userRole === 'Ad Specialist'" />
...
<script>
  import { ref } from 'vue';
  import AdSpeDrawer from "../AdSpe/DrawerItems";
